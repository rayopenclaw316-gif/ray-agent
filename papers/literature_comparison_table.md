...
### 1.3 深度學習方法（DNN-HMM → CNN → RNN+注意力 → Transformer）
...
| Xie et al. (2025) AuxCEMGR | 首個中文神經端對端 sEMG→文字辨識系統，Transformer+CTC+輔助任務 | 8 通道，1238 句 NBA 中文語料，CNN + Transformer + CTC + 拼音生成 + session 對抗分類，CER 38.0% | 1000 Hz | Neuracle NSW308M 雙極系統 | 8（雙極）| ch1(jaw), ch2(orbicularis oris), ch3–8（臉頸部，依 Diener 2015）| BP 10–400 Hz + notch → MFSC（36 Mel）| CNN(×2) + Transformer(×6, 8頭, dim=256) + CTC；輔助：拼音 CTC + session GRL；增強：頻譜相減 + 有聲資料 + mixup | **CER 38.0%**（vs Baseline 44.5%）；1238 句中文 |

> **以下四篇為 sEMG→語音重建（Voice Reconstruction），任務為合成音訊而非辨識文字，屬同類架構的平行研究分支。**

| 作者（年） | 研究貢獻與目標 | 摘要（一句話） | EMG 採樣率 | 設備 | 通道數 | 電極位置 | 訊號前處理 | 模型（含參數） | 主要結果 |
|-----------|-------------|------------|-----------|------|--------|---------|----------|-------------|--------|
| Li et al. (2022) SSRNet | 首個普通話聲調語言 sEMG→語音重建，聲調素輔助任務 | 5 通道浙大設備，TD+STFT 特徵，FFT Transformer Seq2Seq + 聲調素輔助分類，主觀 CER 6.41% | 2000 Hz | 自製（浙大 Guang Li 組）| 5 | 鼻右/左、嘴角右、下巴左角、下巴後 | Butterworth BP 4–400 Hz + 自調式陷波 | FFT Encoder(6 層, dim=384) + DTW 時長 + FFT Decoder + PWG；聲調素輔助(λ=0.5) | **CER 6.41%**（主觀）；移除聲調輔助 → CER +132.75% |
| Li et al. (2023) silentVC | 輔助有聲說話者跨說話者架構 + Conformer 編碼器，跨說話者普通話 sEMG→語音 | 5 通道，4 主×2 輔說話者，Conformer + Length Regulator + Transformer 解碼器，ASR CER 10.69% | 2000 Hz | 自製（浙大，同 Li 2022）| 5 | 同浙大設定（臉部與頸部）| RC 濾波 + Butterworth BP + 梳狀濾波 | Conformer-silentVC(6 塊) + Length Regulator + Transformer 解碼器(6 塊) + PWG | ASR CER **10.69%**；人工 CER 5.31%；MOS 3.95 |
...
> **本節三篇研究的核心問題為「電極放哪裡、放幾個」，與 sEMG-SSR 的辨識架構設計相互獨立。**  
> **Chen (2023)、Song (2023)、Liu (2026) 亦與本節相關，見一節 1.4 Seq2Seq 區塊。**

| 作者（年） | 研究貢獻與目標 | 摘要（一句話） | 採樣率 | 設備 | 通道數（原始→分析）| 電極類型 | 目標肌群 | 分析方法 | 主要電極發現 |
|-----------|-------------|------------|------|------|-----------------|---------|---------|---------|------------|
| Wand et al. (2013) | 首篇引入電極陣列（vs 傳統散點電極）於 EMG 語音辨識；PCA 降維解決維度詛咒；ICA 源分離 | 16/35 通道陣列，audible 英語 108 詞，PCA 前多通道反使性能劣化；PCA 後最佳 WER 10.9%；ICA 最高相對改善 22.9% | 2048 Hz | OT Bioelettronica EMG-USB2 | Setup A: 16 / Setup B: 35（bipolar 差分）| 陣列電極（1×8 + 4×8）；bipolar | 臉頰（顴大肌等發音肌群）+ 下巴（舌肌）；**無頸部電極** | PCA + LDA + ICA + HMM（audible speech）| 35 通道**不加 PCA 反比 16 通道更差**；PCA 相對改善 10–18%；ICA 相對改善最高 22.9%；最佳 WER 10.9% |
...
## 三、1.2.3 節：中文語音辨識與音韻輔助任務

> **本節論文的核心問題為中文聲調語言的挑戰，以及音韻輔助任務的設計。**  
> **Chen (2023)、Song (2023) 亦與本節相關（USTC 中文 33 片語），見一節 1.4 Seq2Seq 區塊。**
...
| Li et al. (2022) SSRNet ★重建 | 首個普通話聲調語言 sEMG→語音重建，聲調素輔助任務量化聲調重要性 | 聲調素輔助分類（139 類）移除後 CER +132.75%；量化證明 sEMG 聲調資訊的可辨識性 | 2000 Hz | 自製（浙大）| 5 | 鼻右/左、嘴角右、下巴左角、頦下 | Butterworth BP + 陷波 | FFT Transformer Seq2Seq + 聲調素輔助 | CER **6.41%**（主觀）；聲調素分類 96.07% |
| Yang et al. (2022) ★EEG | 首次以 EEG RASM 區分有聲調/無聲調普通話，跨受試者 98.82% | 14 受試者 64 通道 EEG，RASM（左右半球 DE 比值）+ BVA 降維 + LDA，說話前差異即已存在 | 1000 Hz（→500 Hz）| SynAmps，64 Ag/AgCl | 64（→8 最佳：F7/F8, C5/C6, P5/P6, O1/O2）| 頭皮（額、中央、頂、枕葉）| BP 0.5–180 Hz + 降採樣 + 偽差去除 | RASM + BVA(14 特徵) + LDA；6 頻段 | 跨受試者 **98.82%**；Before Speak 97.72%（說話前差異已存在）|

---

## 四、1.2.4 節：端對端模型演進（CTC 理論與應用）

> **本節論文提供 CTC 框架的方法論背景，均為一般語音辨識（音訊→文字）或 sEMG 架構演進的重要節點。**
...
| 2048 Hz | Wand (2013), Zhu (2021), Salomons (2025) |
| 未明（高）| Meltzner (2018) |
