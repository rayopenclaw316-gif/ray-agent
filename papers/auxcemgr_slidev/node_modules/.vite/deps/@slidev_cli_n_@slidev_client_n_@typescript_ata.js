//#region node_modules/@typescript/ata/dist/index.js
...
var getNPMVersionsForModule = (config, moduleName) => {
	return api(config, `https://data.jsdelivr.com/v1/package/npm/${moduleName}`, { cache: "no-store" });
};
var getNPMVersionForModuleReference = (config, moduleName, reference) => {
	return api(config, `https://data.jsdelivr.com/v1/package/resolve/npm/${moduleName}@${reference}`);
};
var getFiletreeForModuleWithVersion = (config, moduleName, version) => __async(void 0, null, function* () {
	const res = yield api(config, `https://data.jsdelivr.com/v1/package/npm/${moduleName}@${version}/flat`);
	if (res instanceof Error) return res;
	else return __spreadProps(__spreadValues({}, res), {
		moduleName,
		version
	});
});
var getDTSFileForModuleWithVersion = (config, moduleName, version, file) => __async(void 0, null, function* () {
	...
	const res = yield (config.fetcher || fetch)(url);
	if (res.ok) return res.text();
	else return /* @__PURE__ */ new Error("OK");
});
function api(config, url, init) {
	return (config.fetcher || fetch)(url, init).then((res) => {
		if (res.ok) return res.json().then((f2) => f2);
		else return /* @__PURE__ */ new Error("OK");
	});
}
var mapModuleNameToModule = (moduleSpecifier) => {
	if (moduleSpecifier.indexOf("node:") === 0 || [
		"assert",
		"assert/strict",
		"async_hooks",
		"buffer",
		...
		"worker_threads",
		"zlib"
	].includes(moduleSpecifier)) return "node";
...
var setupTypeAcquisition = (config) => {
	...
	return (initialSourceFile) => {
		...
		return resolveDeps(initialSourceFile, 0).then((t) => {
			var _a, _b;
			if (estimatedDownloaded > 0) (_b = (_a = config.delegate).finished) == null || _b.call(_a, fsMap);
		});
	};
	function resolveDeps(initialSourceFile, depth) {
		return __async(this, null, function* () {
			...
			const treesOnly = (yield Promise.all(depsToGet.map((f) => getFileTreeForModuleWithTag(config, f.module, f.version)))).filter((t) => !("error" in t));
			...
			const dtTreesOnly = (yield Promise.all(mightBeOnDT.map((f) => getFileTreeForModuleWithTag(config, `@types/${getDTName(f.moduleName)}`, "latest")))).filter((t) => !("error" in t));
			...
			if (allDTSFiles.length && depth === 0) (_b = (_a = config.delegate).started) == null || _b.call(_a);
			for (const tree of treesOnly) {
				...
				if (typeof pkgJSON == "string") {
					...
					(_d = (_c = config.delegate).receivedFile) == null || _d.call(_c, pkgJSON, path);
				} else (_e = config.logger) == null || _e.error(`Could not download package.json for ${tree.moduleName}`);
			}
			yield Promise.all(allDTSFiles.map((dts) => __async(this, null, function* () {
				...
				estimatedDownloaded++;
				if (dtsCode instanceof Error) (_a2 = config.logger) == null || _a2.error(`Had an issue getting ${dts.path} for ${dts.moduleName}`);
				else {
					fsMap.set(dts.vfsPath, dtsCode);
					(_c2 = (_b2 = config.delegate).receivedFile) == null || _c2.call(_b2, dtsCode, dts.vfsPath);
					if (config.delegate.progress && estimatedDownloaded % 5 === 0) config.delegate.progress(estimatedDownloaded, estimatedToDownload);
...
function treeToDTSFiles(tree, vfsPrefix) {
	const dtsRefs = [];
	for (const file of tree.files) if (isDtsFile(file.name)) dtsRefs.push({
...
var getReferencesForModule = (ts, code) => {
	...
	return meta.referencedFiles.concat(meta.importedFiles).concat(meta.libReferenceDirectives).filter((f) => !isDtsFile(f.fileName)).filter((d) => !libMap.has(d.fileName)).map((r) => {
		let version = void 0;
...
function getNewDependencies(config, moduleMap, code) {
	return getReferencesForModule(config.typescript, code).map((ref) => __spreadProps(__spreadValues({}, ref), { module: mapModuleNameToModule(ref.module) })).filter((f) => !f.module.startsWith(".")).filter((m) => !moduleMap.has(m.module));
}
...
function getDTName(s) {
	if (s.indexOf("@") === 0 && s.indexOf("/") !== -1) s = s.substr(1).replace("/", "__");
	return s;
}
...
//#endregion
export { getFileTreeForModuleWithTag, getNewDependencies, getReferencesForModule, setupTypeAcquisition };

//# sourceMappingURL=@slidev_cli_n_@slidev_client_n_@typescript_ata.js.map